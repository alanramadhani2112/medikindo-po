            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('web.payments.store.incoming') }}" method="POST" enctype="multipart/form-data">
                        ...
                        {{-- Partial Amount Input - ONLY SHOW FOR PARTIAL PAYMENT --}}
                        ...
                        {{-- 5b. Giro/Cek → Nomor Giro/Cek + Tanggal Jatuh Tempo + Bank Penerbit + No. Referensi --}}
                        ...
                        {{-- 5d. Bank Transfer/VA → No. Referensi Transfer --}}
                        <div :style="!showBankDropdown ? 'display: none;' : ''">
                            <div class="mb-8">
...
function paymentEntryForm() {
    return {
        ...
        outstanding: 0,
        partialAmount: '',
        invoices: {!! json_encode($invoices->map(function($i) {
            return [
                'id' => $i->id,
        ...
        init() {
            ...
            if (oldPaymentType) {
                this.paymentType = oldPaymentType;
            }
            if (oldPaymentMethod) {
                this.paymentMethod = oldPaymentMethod;
            }
            if (oldAmount) {
                this.partialAmount = oldAmount;
            }
        ...
        selectInvoice() {
            ...
            if (inv) {
                ...
                // Don't reset partialAmount if it's already set (from old value)
                if (this.paymentType === 'full') {
                    this.partialAmount = '';
                }
            } else {
