            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-20">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-2xl bg-primary flex items-center justify-center ui-card-title text-white shadow-xl shadow-primary/20">M</div>
                        <span class="text-xl font-bold tracking-tight text-gray-900">Medikindo <span class="text-primary">PO</span></span>
                    </div>
                    
                    <div class="flex items-center gap-4">
                        @auth
                            <a href="{{ route('web.dashboard') }}" class="text-sm font-bold text-slate-600 hover:text-brand-600 transition-colors">Dashboard</a>
                        @else
                            <a href="{{ route('login') }}" class="px-6 py-2.5 rounded-2xl bg-brand-600 text-white text-sm font-bold shadow-xl shadow-brand-500/20 hover:bg-brand-700 hover:-translate-y-0.5 transition-all duration-300">
                                Masuk ke Sistem
                            </a>
                        @endauth
                    </div>
        ...
        <main class="flex-grow">
            <div class="relative overflow-hidden pt-20 pb-20 lg:pt-32 lg:pb-32">
                ...
                <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full pointer-events-none -z-10">
                    <div class="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] bg-brand-50 rounded-full blur-3xl opacity-50"></div>
                    <div class="absolute bottom-[-10%] right-[-10%] w-[30%] h-[30%] bg-brand-50 rounded-full blur-3xl opacity-50"></div>
                </div>
                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="text-center">
                        ...
                        <h1 class="text-5xl lg:text-7xl font-bold text-gray-900 tracking-tight mb-8">
                            ...
                            <span class="text-transparent bg-clip-text bg-gradient-to-r from-primary to-teal-500">Lebih Cerdas & Cepat.</span>
                        </h1>
                        
                        <p class="max-w-2xl mx-auto text-xl text-gray-500 leading-relaxed mb-12">
                            Sistem terintegrasi untuk pengelolaan Purchase Order, Inventaris, dan Laporan Keuangan bagi seluruh jaringan Klinik dan Rumah Sakit Medikindo.
                        </p>

                        <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                            @auth
                                <x-button href="{{ route('web.dashboard') }}" size="xl" variant="primary" class="w-full sm:w-auto">
                                    Buka Dashboard
                                </x-button>
                            @else
                                <x-button href="{{ route('login') }}" size="xl" variant="primary" class="w-full sm:w-auto px-10">
                                    Mulai Sekarang
                                </x-button>
                        ...
                        <div class="mt-20 grid grid-cols-2 lg:grid-cols-4 gap-8 border-t border-slate-100 pt-16">
                            @foreach([
                                ['label' => 'Total PO Terbit', 'value' => '1.2k+'],
                                ['label' => 'Klinik Terintegrasi', 'value' => '48'],
                                ['label' => 'Supplier Aktif', 'value' => '150+'],
                                ['label' => 'SLA Persetujuan', 'value' => '< 2 Jam'],
                            ] as $stat)
