<x-layout title="Buat PO" pageTitle="Buat Purchase Order" breadcrumb="Isi form untuk membuat pesanan pembelian baru">
            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('poForm', () => ({
                    ...
                    loadProducts(isInit = false) {
                        ...
                        const option = select ? Array.from(select.options).find(o => o.value == this
                            .supplierId) : null;
                        try {
                            this.products = option && option.dataset.products ? JSON.parse(option.dataset
                                .products) : [];
                        } catch (e) {
                            this.products = [];
                        }
    ...
    <div
        class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-4 mb-7">
        <div>
    ...
    <form method="POST" action="{{ route('web.po.store') }}" id="po-form" x-data="poForm()"
        x-init="init()">
        @csrf
        ...
        <x-card title="Informasi Pemesanan" icon="information" class="card-flush mb-7">
            <div class="row g-5">
                ...
                <div class="col-md-6">
                    <x-select name="supplier_id" label="Supplier Pemenuhan Item" required x-model="supplierId" @change="loadProducts()">
                        ...
                        @foreach($suppliers as $supplier)
                            <option value="{{ $supplier->id }}" data-products='@json($supplier->products)'>
                ...
                <div class="col-md-6">
                    <x-input name="notes" label="Catatan Tambahan (Opsional)" value="{{ old('notes') }}"
            ...
            <x-slot name="actions">
                <div class="d-block">
                    <button type="button" class="btn btn-sm btn-primary d-flex align-items-center gap-2"
                        @click="addItem()" :disabled="!supplierId" :class="{ 'opacity-50': !supplierId }">
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-10">
                                    <div class="d-flex flex-column align-items-center">
                                        ...
                                        <p class="text-gray-600 fs-7 mb-0">Pilih supplier, kemudian tekan "Tambah
                                            Produk".</p>
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <div class="product-search-wrapper">
                                        ...
                                        <div class="position-relative">
                                            ...
                                            <input type="text" class="form-control form-control-solid ps-12"
                                                placeholder="Ketik nama atau SKU produk..." x-model="searchQuery[index]"
                                                x-bind:data-search-input="index" @focus="openSearch(index)"
                                        ...
                                        <div class="product-dropdown" x-show="showDropdown[index]"
                                            x-bind:data-search-dropdown="index" x-cloak>
                                                ...
                                                <div class="product-dropdown-item"
                                                    ...
                                                    <div class="text-muted fs-8">
                                                        ...
                                                        <span class="ms-3 text-primary fw-semibold"
                                                            x-text="'Rp ' + (Number(p.cost_price || p.price || 0)).toLocaleString('id-ID')"></span>
                                                    </div>
                                                </div>
                                            </template>
                                        </div>
                                    ...
                                    <div x-show="item.product_id" class="mt-1">
                                        <span class="badge badge-light-primary fs-8"
                                            x-text="item.product_sku ? 'SKU: ' + item.product_sku : ''"></span>
                                    </div>
                                ...
                                <td class="px-4 py-3">
                                    <input type="number" x-bind:name="'items[' + index + '][quantity]'"
                                        x-bind:value="item.quantity" @input="updateQuantity(index, $event.target.value)"
                                        min="1" class="form-control form-control-solid" />
                                </td>

                                {{-- Unit Price --}}
                                <td class="px-4 py-3">
                                    <input type="number" x-bind:name="'items[' + index + '][unit_price]'"
                                        x-bind:value="item.unit_price" readonly
                                ...
                                <td class="px-4 py-3 text-end align-middle">
                                    <span class="text-gray-900 fw-bold fs-6"
                                        x-text="formatRupiah(item.subtotal)"></span>
                                </td>
        ...
        <div class="d-flex align-items-center justify-content-end gap-3 pt-4">
            ...
            <x-button type="submit" variant="primary" size="lg" class="create-confirm"
