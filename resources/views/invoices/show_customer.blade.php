    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-7">
            <div class="d-flex flex-column">
                ...
                <p class="text-gray-600 fs-6 mb-0">Tagihan Kepada: <span
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="text-success fs-2 fw-bold mt-2">Rp
            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="text-gray-900 fs-2 fw-bold mt-2">Rp
                        ...
                        <div class="d-flex flex-column gap-3">
                            ...
                            @if ($invoice->goods_receipt_id)
                                <a href="{{ route('web.goods-receipts.show', $invoice->goods_receipt_id) }}"
                                    ...
                                    <div class="d-flex flex-column">
                                        ...
                                        <span
                                            class="text-gray-900 fw-bold">{{ $invoice->goodsReceipt?->gr_number ?? '—' }}</span>
                                    </div>
                            ...
                            <a href="{{ route('web.po.show', $invoice->purchase_order_id) }}"
                                ...
                                <div class="d-flex flex-column">
                                    ...
                                    <span
                                        class="text-gray-900 fw-bold">{{ $invoice->purchaseOrder?->po_number ?? '—' }}</span>
                                </div>
                            ...
                            @if ($invoice->purchaseOrder?->external_po_number)
                                <div class="d-flex align-items-center justify-content-between p-3 rounded bg-light-info">
                                    <div class="d-flex flex-column">
                                        ...
                                        <span
                                            class="text-gray-900 fw-bold">{{ $invoice->purchaseOrder->external_po_number }}</span>
                                    </div>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($item->expiry_date)
                                            @php
                                                $isExpiringSoon =
                                                    $item->expiry_date->diffInDays(now()) <= 90 &&
                                                    $item->expiry_date->isFuture();
                                                $isExpired = $item->expiry_date->isPast();
                                            @endphp
                                            <span
                                                class="badge badge-light-{{ $isExpired ? 'danger' : ($isExpiringSoon ? 'warning' : 'success') }}">
                                    ...
                                    <td class="text-center">
                                        @if ($item->discount_percentage > 0)
                                            <span
                                                class="badge badge-light-warning">{{ number_format($item->discount_percentage, 1) }}%</span>
                                        @else
                                            <span class="text-muted">—</span>
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="9" class="text-center py-10">
                                        <div class="d-flex flex-column align-items-center">
                                            ...
                                            <span class="text-gray-500 fs-6">Tidak ada rincian barang untuk invoice
                                                ini.</span>
                                        </div>
                                    </td>
                                </tr>
                                ...
                                <tbody>
                                    @forelse($invoice->paymentAllocations as $alloc)
                                        <tr>
                                            <td class="ps-4">
                                                <span
                                                    class="text-gray-800 fw-bold">{{ $alloc->payment?->payment_number }}</span>
                                            </td>
                                            <td>
                                                <span
                                                    class="badge badge-light-secondary">{{ strtoupper($alloc->payment?->payment_method) }}</span>
                                            </td>
                                            <td class="text-end">
                                                <span class="text-success fw-bold">+ Rp
                                                    {{ number_format($alloc->allocated_amount, 0, ',', '.') }}</span>
                                            </td>
                                            <td class="text-end pe-4">
                                                <span
                                                    class="text-gray-600">{{ $alloc->created_at->format('d/m/Y') }}</span>
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="4" class="text-center py-10">
                                                <div class="d-flex flex-column align-items-center">
                                                    ...
                                                    <span class="text-gray-700 fs-5 fw-semibold mb-2">Belum Ada
                                                        Pembayaran</span>
                                                    <span class="text-gray-500 fs-6">Pembayaran untuk invoice ini belum
                                                        diterima.</span>
                                                </div>
                                            </td>
                                        </tr>
                        ...
                        <div class="d-flex flex-column gap-3">
                            <div
                                ...
                                <span class="text-gray-900 fw-semibold">Rp
                                    {{ number_format($invoice->subtotal_amount ?? 0, 0, ',', '.') }}</span>
                            </div>
                            ...
                            @if (($invoice->ematerai_fee ?? 0) > 0)
                                <div
                                    ...
                                    <span class="text-gray-900 fw-semibold">Rp
                                        {{ number_format($invoice->ematerai_fee, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            <div
                                class="d-flex justify-content-between align-items-center py-3 px-4 rounded bg-light-primary mt-2">
                            ...
                            <div
                                class="d-flex justify-content-between align-items-center py-3 px-4 rounded {{ $invoice->total_amount - $invoice->paid_amount > 0 ? 'bg-light-danger' : 'bg-light-success' }}">
                                <span
                                    class="fw-bold fs-6 {{ $invoice->total_amount - $invoice->paid_amount > 0 ? 'text-danger' : 'text-success' }}">Sisa
                                    Tagihan</span>
                                <span
                                    class="fw-bold fs-5 {{ $invoice->total_amount - $invoice->paid_amount > 0 ? 'text-danger' : 'text-success' }}">
