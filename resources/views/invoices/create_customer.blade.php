<x-layout title="Buat Tagihan ke RS/Klinik" pageTitle="Buat Tagihan ke RS/Klinik"
    breadcrumb="Buat tagihan berdasarkan penerimaan barang">

    <x-page-header title="Buat Tagihan ke RS/Klinik"
        ...
        $grData = $goodsReceipts
            ->map(
                fn($gr) => [
                    ...
                    'items' => $gr->items
                        ->map(function ($item) {
                            $resolvedProduct = $item->purchaseOrderItem?->product ?? $item->product;

                            return [
                                'id' => $item->id,
                                'product_name' => $resolvedProduct?->name ?? '—',
                                'product_unit' => $resolvedProduct?->unit ?? 'unit',
                                'batch_no' => $item->batch_no,
                                'expiry_date' => $item->expiry_date?->format('Y-m-d'),
                                'quantity_received' => $item->quantity_received,
                                'remaining_quantity' => $item->remaining_ar_quantity,
                                'unit_price' =>
                                    (float) ($resolvedProduct?->selling_price ??
                        ...
                        <div class="gr-search-wrapper">
                            ...
                            <div class="position-relative">
                                ...
                                <input type="text" class="form-control form-control-solid ps-12"
                                    placeholder="Ketik Nomor GR atau Nama RS/Klinik..." x-model="searchQuery"
                                ...
                                <template x-for="gr in filteredGrs()" :key="gr.id">
                                    <div class="gr-dropdown-item" @mousedown.prevent="selectGr(gr)">
                                        ...
                                        <div class="text-muted fs-8">
                                            ...
                                            <span class="ms-2 badge badge-light-info fs-9"
                            ...
                            <div class="gr-dropdown" x-show="showDropdown && searchQuery && filteredGrs().length === 0"
                                x-cloak>
                                <div class="gr-dropdown-empty">Data GR tidak ditemukan</div>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <tbody>
                                <template x-for="(item, index) in items" :key="item.id">
                                    <tr>
                                        ...
                                        <td class="text-end text-success fw-bold" x-text="item.remaining_quantity">
                                        </td>
                                        <td class="text-end">
                                        ...
                                        <td class="text-end fw-bold text-gray-900"
                                            x-text="formatCurrency(item.unit_price * item.invoice_quantity)"></td>
                                    </tr>
                                </template>
                            </tbody>
                        </table>
                    ...
                    <div class="d-flex justify-content-end mt-5">
                        <div class="w-100 w-md-400px">
                            <div class="d-flex flex-stack mb-3">
                                ...
                                <div class="fw-bold text-gray-800 fs-6" x-text="formatCurrency(summary.subtotal)">
                                </div>
                            </div>
                            <div class="d-flex flex-stack mb-3">
                            ...
                            <div class="d-flex flex-stack mb-3" x-show="summary.ematerai > 0">
                                ...
                                <div class="fw-bold text-gray-800 fs-6" x-text="formatCurrency(summary.ematerai)">
                                </div>
                            </div>
                            <div class="separator separator-dashed my-3"></div>
                            <div class="d-flex flex-stack">
                                <div class="fw-bold text-gray-800 fs-4">Total Tagihan:</div>
                                <div class="fw-bolder text-gray-900 fs-3" x-text="formatCurrency(summary.grandTotal)">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                return {
                    ...
                    selectGr(gr) {
                        ...
                        this.items = gr.items.filter(i => i.remaining_quantity > 0).map(i => ({
                            ...i,
                            invoice_quantity: i.remaining_quantity
                        }));
