    ...
    <div class="card mb-5">
        <div class="card-body">
            <form action="{{ route('web.invoices.index') }}" method="GET" class="d-flex flex-wrap gap-3">
                ...
                @if(request('search'))
                    <a href="{{ route('web.invoices.index', array_merge(request()->except(['search', 'page']), ['tab' => 'supplier'])) }}" 
                       class="btn btn-light">
        ...
        <div class="card-header border-0 pt-6 pb-2 d-flex justify-content-between align-items-center">
            ...
            <ul class="nav nav-tabs nav-line-tabs nav-line-tabs-2x nav-stretch fs-6 fw-bold border-0">
                ...
                @foreach($tabStatus as $val => $tabData)
                    ...
                    @endphp
                    <li class="nav-item">
                        <a href="{{ route('web.invoices.index', array_merge(request()->except(['status', 'page']), ['tab' => 'supplier', 'status' => $val === '' ? null : $val])) }}" 
                            ...
                            <tr>
                                <td class="ps-4">
                                    <div class="d-flex flex-column">
                                        <a href="{{ route('web.invoices.supplier.show', $inv) }}" 
                                           class="text-gray-900 text-hover-primary fw-bold fs-6">
                                            {{ $inv->invoice_number }}
                                        </a>
                                        <span class="text-gray-500 fs-7 mt-1">Ref: {{ $inv->purchaseOrder?->po_number ?? '—' }}</span>
                                    </div>
                                </td>
                                <td>
                                    <span class="text-gray-800 fw-semibold">{{ $inv->supplier?->name ?? '—' }}</span>
                                </td>
                                <td>
                                    <div class="d-flex flex-column">
                                        <span class="text-gray-800 fw-semibold {{ $inv->isOverdue() ? 'text-danger' : '' }}">
                                            {{ $inv->due_date?->format('d M Y') ?? '—' }}
                                        </span>
                                        @if(!$inv->isPaid() && $inv->due_date)
                                            <span class="text-gray-500 fs-7 mt-1">{{ $inv->due_date->diffForHumans() }}</span>
                                        @endif
                                    </div>
                                </td>
                                <td class="text-end">
                                    <div class="d-flex flex-column">
                                        <span class="text-gray-900 fw-bold fs-6">Rp {{ number_format($inv->total_amount, 0, ',', '.') }}</span>
                                        <span class="text-primary fs-7 fw-semibold mt-1">Dibayar: Rp {{ number_format($inv->paid_amount, 0, ',', '.') }}</span>
                                    </div>
                                </td>
                                <td>
                                    @php
                                    ...
                                    @endphp
                                    <span class="badge badge-{{ $statusColor }}">{{ strtoupper($inv->status) }}</span>
                                </td>
                                <td class="text-end pe-4">
                                    <div class="d-flex justify-content-end">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <a href="{{ route('web.invoices.supplier.show', $inv) }}" class="dropdown-item">
