<x-index-layout title="Tagihan ke RS/Klinik (AR)" description="Kelola tagihan yang diterbitkan ke RS/Klinik" :breadcrumbs="[['label' => 'Account Receivable']]">
        ...
        <x-filter-bar :action="route('web.invoices.customer.index')">
            <div class="flex-grow-1" style="max-width: 300px;">
                <div class="position-relative">
                    ...
                    <input type="text" name="search" class="form-control form-control-solid ps-12" placeholder="No. Invoice / Nama RS..." value="{{ request('search') }}">
                </div>
            </div>
            ...
            @forelse($invoices as $invoice)
                <tr>
                    ...
                    <td class="text-center">
                        ...
                        @endphp
                        <span class="badge badge-light-{{ $badgeClass }} fw-bold">{{ strtoupper(str_replace('_', ' ', $invoice->status)) }}</span>
                    </td>
                    <td class="text-end">
                        <a href="{{ route('web.invoices.customer.show', $invoice) }}" class="btn btn-icon btn-light-primary btn-sm" title="Lihat Detail">
                            <i class="ki-outline ki-eye fs-2"></i>
                        </a>
                        @if($invoice->status === 'draft')
                            <form method="POST" action="{{ route('web.invoices.customer.issue', $invoice) }}" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-icon btn-light-success btn-sm" title="Terbitkan Invoice">
                                    <i class="ki-outline ki-check-circle fs-2"></i>
                                </button>
                            </form>
                        @endif
                        <a href="{{ route('web.invoices.customer.pdf', $invoice) }}" target="_blank" class="btn btn-icon btn-light-info btn-sm" title="Cetak PDF">
            ...
            @empty
                <tr>
                    <td colspan="7" class="text-center py-10">
                        <x-empty-state icon="document" title="Belum Ada Tagihan" message="Tagihan AR dibuat otomatis saat Supplier Invoice diverifikasi." />
                    </td>
                </tr>
            @endforelse
        </tbody>
