        ...
        <div class="col-lg-8">
            <x-card title="Informasi Pembayaran" icon="bill">
                <div class="row mb-7">
                    ...
                    <div class="col-lg-8">
                        <span
                            class="fw-bold fs-6 text-gray-800">{{ $paymentProof->customerInvoice->invoice_number }}</span>
                        <a href="{{ route('web.invoices.customer.show', $paymentProof->customerInvoice) }}"
                            class="ms-2">
                ...
                <div class="row mb-7">
                    ...
                    <div class="col-lg-8 fv-row">
                        <span
                            class="fw-bold fs-6 text-gray-800">{{ $paymentProof->customerInvoice->organization->name }}</span>
                    </div>
                </div>
                ...
                <div class="row mb-7">
                    ...
                    <div class="col-lg-8">
                        <span
                            class="fw-bold fs-6 text-gray-800">{{ $paymentProof->payment_date->format('d F Y') }}</span>
                    </div>
                </div>
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="ms-1">
                                            <a href="{{ route('web.payment-proofs.download-document', [$paymentProof, $doc->id]) }}"
                                                class="fs-6 text-gray-800 text-hover-primary fw-bold">{{ $doc->original_filename }}</a>
                                            <div class="fs-7 text-muted fw-semibold">
                                                {{ number_format($doc->file_size / 1024, 2) }} KB</div>
                                        </div>
                                    </div>
                                    <a href="{{ route('web.payment-proofs.download-document', [$paymentProof, $doc->id]) }}"
        ...
        <div class="col-lg-4">
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <div class="d-flex align-items-center">
                            <div class="symbol symbol-35px me-3">
                                <span
                                    class="symbol-label bg-light-primary text-primary fw-bold">{{ substr($paymentProof->submittedBy->name, 0, 1) }}</span>
                            </div>
                            <div class="d-flex flex-column">
                                <span class="text-gray-800 fw-bold fs-7">{{ $paymentProof->submittedBy->name }}</span>
                                <span
                                    class="text-muted fs-7">{{ $paymentProof->created_at->format('d M Y H:i') }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    @if ($paymentProof->verifiedBy)
                        <div class="d-flex flex-column">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="symbol symbol-35px me-3">
                                    <span
                                        class="symbol-label bg-light-info text-info fw-bold">{{ substr($paymentProof->verifiedBy->name, 0, 1) }}</span>
                                </div>
                                <div class="d-flex flex-column">
                                    <span
                                        class="text-gray-800 fw-bold fs-7">{{ $paymentProof->verifiedBy->name }}</span>
                                    <span
                                        class="text-muted fs-7">{{ $paymentProof->verified_at->format('d M Y H:i') }}</span>
                                </div>
                            </div>
                        </div>
                    @endif
                    ...
                    @if ($paymentProof->approvedBy)
                        <div class="d-flex flex-column">
                            ...
                            <div class="d-flex align-items-center">
                                <div class="symbol symbol-35px me-3">
                                    <span
                                        class="symbol-label bg-light-success text-success fw-bold">{{ substr($paymentProof->approvedBy->name, 0, 1) }}</span>
                                </div>
                                <div class="d-flex flex-column">
                                    <span
                                        class="text-gray-800 fw-bold fs-7">{{ $paymentProof->approvedBy->name }}</span>
                                    <span
                                        class="text-muted fs-7">{{ $paymentProof->approved_at->format('d M Y H:i') }}</span>
                                </div>
                            </div>
                        </div>
                    @endif
            ...
            @can('uploadDocument', $paymentProof)
                <div class="card card-flush shadow-sm mt-5">
                    ...
                    <div class="card-body">
                        <form action="{{ route('web.payment-proofs.upload-document', $paymentProof) }}" method="POST"
