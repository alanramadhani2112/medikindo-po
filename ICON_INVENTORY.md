...
## 📋 Daftar Lengkap Icon Berdasarkan Kategori

### 🎯 Navigation & UI Controls (9 icons)
...
## 📊 Statistik Penggunaan Icon

### Icon Paling Sering Digunakan (Top 10)

1. **check** - Button konfirmasi/simpan (±50 kali)
2. **cross** - Button batal/tutup (±45 kali)
3. **plus** - Button tambah data (±30 kali)
4. **pencil** - Button edit (±28 kali)
5. **trash** - Button hapus (±25 kali)
6. **eye** - Button lihat detail (±22 kali)
7. **magnifier** - Search/filter (±20 kali)
8. **check-circle** - Success message (±18 kali)
9. **cross-circle** - Error message (±15 kali)
10. **dots-horizontal** - Action menu (±12 kali)

### Distribusi Icon per Modul

| Modul | Jumlah Icon Unik | Icon Utama |
|-------|------------------|------------|
| Dashboard | 5 | element-11, chart, chart-line-up |
| Purchase Orders | 8 | purchase, basket, check-square |
| Approvals | 6 | check-square, check-circle, cross-circle |
| Goods Receipt | 7 | package, delivery, document |
| Invoicing (AR/AP) | 10 | arrow-up, arrow-down, bill, dollar |
| Payments | 8 | wallet, dollar, calculator, verify |
| Credit Control | 6 | chart-simple, shield-tick, shield-cross |
| Organizations | 7 | office-bag, bank, hospital, shop |
| Suppliers | 6 | delivery-3, phone, geolocation |
| Products | 5 | capsule, package, note |
| Users | 6 | profile-user, user, user-tick, lock |
...
| Size Class | Ukuran | Penggunaan |
|------------|--------|------------|
| `fs-1` | Extra Large | Hero icons |
| `fs-2` | Large | Button icons, menu icons |
| `fs-3` | Medium | Inline icons, dropdown |
| `fs-4` | Small | Table actions, badges |
| `fs-2x` | 2x | Dashboard cards |
| `fs-3x` | 3x | Empty states |

### Warna Icon

| Color Class | Warna | Penggunaan |
|-------------|-------|------------|
...
| `text-gray-400` | Gray | Empty state |
| `text-gray-500` | Dark Gray | Disabled |
...
### Pagination
- Previous: `ki-black-left`
- Next: `ki-black-right`

---

## 🔄 Icon yang Perlu Diganti (Rekomendasi)

Jika Anda ingin mengganti icon, berikut rekomendasi berdasarkan konsistensi:

### Prioritas Tinggi (Sering Digunakan)
1. ✅ `ki-check` → Sudah konsisten
2. ✅ `ki-cross` → Sudah konsisten
3. ✅ `ki-plus` → Sudah konsisten
4. ✅ `ki-pencil` → Sudah konsisten
...
### Icon yang Jarang Digunakan
- `ki-abstract-26` → Bisa dihapus jika tidak terpakai
- `ki-home-2` → Duplikat dengan `ki-home`
- `ki-note-2` → Duplikat dengan `ki-note`
...
Jika ingin mengganti icon, pertimbangkan:

1. **Konsistensi**: Gunakan icon dari library yang sama
2. **Ukuran**: Pertahankan ukuran yang sudah ada (fs-2, fs-3, fs-4)
3. **Warna**: Pertahankan color scheme yang sudah ada
4. **Semantik**: Icon harus sesuai dengan fungsinya
5. **Accessibility**: Pastikan icon mudah dipahami

**Apakah Anda ingin mengganti icon tertentu atau semua icon dengan library lain?**
